    ...
    switch (message) {
      ...
      case ServerAfterPlayerInventoryChangeMessage msg:
        if (_agent is not null) {
          foreach (var changeInfo in msg.ChangeList) {
            if (_agent.UniqueId != changeInfo.PlayerUniqueId) {
              continue;
            }

            Inventory inventory = (Inventory)_agent.Inventory;
            foreach (var slotChangeInfo in changeInfo.ChangeList) {
              inventory[slotChangeInfo.Slot] = slotChangeInfo.ItemTypeId is null ? null :
                new ItemStack(slotChangeInfo.ItemTypeId.Value, slotChangeInfo.Count);
            }
          }
        }
        break;
