...
public class Recorder : IRecorder, IDisposable {
  ...
  private readonly NovelCraft.Utilities.Logger.Logger _logger = new("Server.Recorder");
    ...
    if (Monitor.TryEnter(_saveLock)) {
      try {
        lock (_saveLock) {
          ...
          JsonNode recordJson = this.Json;

          _recordQueue.Clear();

          // Use timestamp as file name.
          long timestamp = (long)DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1)).TotalMicroseconds;
